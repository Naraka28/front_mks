      ...
      <div className="bg-black bg-opacity-45 fixed inset-0 z-10 w-screen overflow-y-auto flex items-center justify-center">
        <DialogPanel className="bg-white border-4 border-gray-300 p-3 rounded-lg shadow-2xl w-[42rem] h-auto overflow-y-auto">
          ...
          <div className="p-2 mr-10 flex justify-center items-center">
            ...
            <DialogTitle className="text-4xl font-bold text-center">Detalles de la Leche</DialogTitle>
          </div>
          <div className="flex items-center justify-center border-t-2 border-gray-200 pt-1 mb-6 mt-4">
            <div className="p-2 mr-10 bg-purple-100 rounded-full mt-4">
              <img src={milk.image} alt="" height="200rem" width="200rem" />
            </div>
            <div className="gap-1 mt-4">
              <p className="text-xl text-gray-700"><strong>Leche:</strong> {milk.name}</p>
              <p className="text-xl text-gray-700"><strong>Precio:</strong> ${milk.price}</p>
            </div>
          </div>
        </DialogPanel>
      </div>
...
const DeleteConfirmationModal = ({ open, onClose, milk, onConfirm }) => {
  if (!milk) return null;
      ...
      <div className="bg-black bg-opacity-45 fixed inset-0 z-10 w-screen overflow-y-auto flex items-center justify-center">
        <DialogPanel className="bg-white border-4 border-gray-300 p-3 rounded-lg shadow-2xl w-[38rem] h-auto overflow-y-auto">
          ...
          <div className="text-center mb-4">
            <p className="text-xl text-gray-700 mb-2">¿Estás seguro que deseas eliminar la leche?</p>
            <p className="text-2xl font-bold text-gray-800">"{milk.name}"</p>
            <div className="p-2 bg-purple-100 flex justify-center items-center rounded-full mt-2">
              <img src={milk.image} alt="" className="max-w-full h-auto object-contain max-h-60 rounded-md"/>
            </div>
...
const MilksTable = () => {
  ...
  const { data: milksList = [], isLoading, error } = useQuery({
    queryKey: ['milks'],
    queryFn: getMilks,
  });
  ...
  return (
    <div className="p-6 overflow-scroll px-0">
      <table className="mt-4 w-full min-w-max table-auto text-left">
        <thead>
          <tr>
            {['Leche', 'Imagen', 'Precio', 'Acciones'].map((header) => (
      ...
      <DeleteConfirmationModal
        ...
        milk={selectedMilk}
        onConfirm={handleDeleteMilk}
      />
    </div>
  );
};

export default MilksTable;
